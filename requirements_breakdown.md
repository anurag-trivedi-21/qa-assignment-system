...
## Summary: New Files & Migrations to Create

| Type | What |
|---|---|
| Migration | Add `clocked_in_at`, `last_test_submitted_at` to `users` |
| Migration | Add `is_auto_assigned`, `return_to_queue_at` to `pending_tests` |
| Service | `App\Services\ClockService` |
| Job | `App\Jobs\AutoAssignTestsJob` |
| Command | `App\Console\Commands\AutoClockOutInactiveTesters` |
| Config | `config/testing.php` (timeouts and cooldown values) |
| Filament Action | Clock In / Clock Out buttons on Users resource |
| Filament Action | "Run Assignment Now" manual trigger button |
| Tests | `tests/Feature/ClockServiceTest.php` |
| Tests | `tests/Feature/AutoAssignmentTest.php` |
| Tests | `tests/Feature/InactivityTimeoutTest.php` |
